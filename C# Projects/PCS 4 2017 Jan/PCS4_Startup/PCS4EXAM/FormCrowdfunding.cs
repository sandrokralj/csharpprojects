...
namespace PCS4EXAM
{
    public partial class FormCrowdfunding : Form
    {
        ...
        private LinkedListOfBigDonations LL;
...

        {
            ...
            LL = new LinkedListOfBigDonations();
        ...
        private void btnAddStartup_Click(object sender, EventArgs e)
        {
            if (this.cf.getStartup(tbNameStartup.Text) == null)
            {
                ...
                if (temp.Description.Contains("music"))
                {
                    temp.thresholdReached += new StartUp.TresholdReachedHandler(this.frmCapitalStudio.ShowStartupInMyListbox);
                }
                if(temp.Threshold >= 10000)
                {
                    temp.thresholdReached += new StartUp.TresholdReachedHandler(this.frmRedFlat.AlertMe);
                }
                temp.thresholdReached += new StartUp.TresholdReachedHandler(this.congratulations);
        ...
        private void btnAddDonation_Click(object sender, EventArgs e)
        {
            ...
            if (s != null)
            {
                ...
                {
                    this.LL.addDonationToLinkedList(tbNameStartup.Text, tbNameDonator.Text, Convert.ToInt32(tbAmount.Text));
                }
            }
            else
            {
        ...
        {
            //todo, assignment 4
            List<String> temp = this.LL.getInfoOfAllItems();
