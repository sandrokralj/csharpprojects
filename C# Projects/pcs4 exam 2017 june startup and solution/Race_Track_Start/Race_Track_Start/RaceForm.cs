...
namespace Race_Track_Solution
{
    public partial class RaceForm : Form
    {
        ...
        private void InitializeParticipants()
        {
            ...
            p.raceFinished += new Participant.RaceFinishedHandler(showInListBox);
            ...
            myRace.AddParticipant(p);
            p.raceFinished += new Participant.RaceFinishedHandler(showInListBox);
            p = new Participant("Sandro", 17, "Basketball", Color.Black);
            myRace.AddParticipant(p);
            p.raceFinished += new Participant.RaceFinishedHandler(showInListBox);
        }
        ...
        {
            Participant youngest = myRace.YoungestParticipant();
            if (youngest == null)
                MessageBox.Show("no participants, so also no youngest");
            else
                MessageBox.Show(myRace.YoungestParticipant().ToString());
        }
        ...
        public void showInListBox(Participant p)
        {
            ...
            if (this.lbInfo.Items.Count == 1) { this.myLinkedList.registeredWinner(p); }
        }
